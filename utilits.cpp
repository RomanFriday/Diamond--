					...
					s_map* save_map, s_player* save_player, s_q_stone* save_q_stone, s_enemy** save_first_enemy)
{
	...
	*save_player = *player;
	q_stone_clear(save_q_stone); // очищаем старые значени€
					...
					s_map *save_map, s_player *save_player, s_q_stone *save_q_stone, s_enemy **save_first_enemy)
{
	...
	*player = *save_player;
	q_stone_clear(q_stone); // очищаем старые значени€
...
// освободить пам€ть матрицы с типом s_cell. передать количество строк. всегда возвращает 0
...
// очистить всю пам€ть, что занимали. возвращает 0
